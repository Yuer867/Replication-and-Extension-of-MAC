...
def config():
    ...
    parser.add_argument('--dataset', type=str, default='FakeNewsNet_plus',
                        choices=['FakeNewsNet_plus'])
    ...
    parser.add_argument('--relevant_tweets_num', type=int, default=50)

    parser.add_argument('--only_article', action='store_true')
    ...
    parser.add_argument('--mac_nhead_3', type=int, default=3)
    parser.add_argument('--mac_nhead_4', type=int, default=1)
...
def init_dataset(file):
    ...
    for piece in pieces:
        ...
        label = piece[1]['label']
        claim = piece[1]['claim']
        claim_source = piece[1]['speaker']
        evidence = piece[1]['articles']
        evidence_source = piece[1]['article_sources']
        tweet = piece[1]['posts']
        tweet_source = piece[1]['post_sources']

        datasets[claim_id] = {'claim': claim, 'claim_source': claim_source,
                              'evidence': evidence, 'evidence_source': evidence_source,
                              'tweet': tweet, 'tweet_source': tweet_source,
                              'label': label}
    return datasets
...
def get_tweet_source(pieces):
    sources = list()
    for p in pieces:
        sources += pieces[p]['tweet_source']
    sources = set(sources)
    source2idx = {k: i + 1 for i, k in enumerate(sources)}
    return source2idx
...
class DatasetLoader(Dataset):
    def __init__(self, args, pieces, post, article, tweet):
        self.args = args
        ...
        self.post_tweets_tokens = tweets
        self.post_tweets_masks = dict()
        self.post_tweets_sources = dict()
        for post_idx, p in enumerate(tqdm(pieces)):
            tweets = pieces[p]['tweet']
            sz = len(tweets)

            mask = torch.zeros(sz, 1, dtype=torch.float, device=self.args.device)
            if sz != 0:
                mask[:-sz] = 1 / sz
            self.post_tweets_masks[post_idx] = mask

            tweet_sources = pieces[p]['tweet_source']
            tweet_sources = [tweetSource2idx.get(p, 0) for p in tweet_sources]
            self.post_tweets_sources[post_idx] = tweet_sources
...
class MAC(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.relevant_tweets_num = args.relevant_tweets_num

        self.only_article = args.only_article
        self.only_tweet = args.only_tweet

        self.num_post_source = args.num_post_source
        self.num_article_source = args.num_article_source
        self.num_tweet_source = args.num_tweet_source
        ...
        self.num_heads_3 = args.mac_nhead_3
        self.num_heads_4 = args.mac_nhead_4
        self.num_layers = 1
        ...
        self.tweet_source_emb = nn.Embedding(self.num_tweet_source, self.one_hot_dim)
        nn.init.uniform_(self.tweet_source_emb.weight, -0.2, 0.2)
        ...
        self.tweet_bilstm = nn.LSTM(self.input_dim, self.hidden_size, self.num_layers,
                                    bidirectional=True, batch_first=True, dropout=self.dropout_query)
        ...
        self.W7 = nn.Linear(4 * self.hidden_size, 2 *
                            self.hidden_size, bias=False)
        self.W8 = nn.Linear(2 * self.hidden_size, self.num_heads_3, bias=False)

        w3_dim = (self.num_heads_1 + 1) * 2 * self.hidden_size + 2 * self.one_hot_dim
        self.W3 = nn.Linear(w3_dim, 2 * self.hidden_size, bias=False)
        self.W4 = nn.Linear(2 * self.hidden_size, self.num_heads_2, bias=False)

        w9_dim = (self.num_heads_3 + 1) * 2 * self.hidden_size + 2 * self.one_hot_dim
        self.W9 = nn.Linear(w9_dim, 2 * self.hidden_size, bias=False)
        self.W10 = nn.Linear(2 * self.hidden_size, self.num_heads_4, bias=False)

        if self.only_article:
            last_output_dim = 2 * self.hidden_size * (1 + self.num_heads_1 * self.num_heads_2) + \
                              self.one_hot_dim * (1 + self.num_heads_2)
        elif self.only_tweet:
            last_output_dim = 2 * self.hidden_size * (1 + self.num_heads_3 * self.num_heads_3) + \
                              self.one_hot_dim * (1 + self.num_heads_3)
        else:
            last_output_dim = 2 * self.hidden_size * (1 + self.num_heads_1 * self.num_heads_2 + self.num_heads_3 * self.num_heads_4) + \
                              self.one_hot_dim * (1 + self.num_heads_2 + self.num_heads_4)
    ...
    def forward(self, idxs, dataset):
        ...
        for idx in idxs:
            # (#doc, max_doc_len)
            inputs = [self._encode(doc, max_len=self.max_doc_len)
                      for doc in dataset.post_tweets_tokens[idx.item()][:self.relevant_tweets_num]]
            input_ids = torch.tensor(
                [i[0] for i in inputs], dtype=torch.long, device=self.args.device)
            padding_length = self.relevant_tweets_num - len(inputs)
            input_padding = torch.zeros(padding_length, self.max_doc_len, dtype=torch.long, device=self.args.device)
            input_ids = torch.cat((input_ids, input_padding), dim=0)
            ...
            # (#doc)
            sources = dataset.post_tweets_sources[idx.item()][:self.relevant_tweets_num]
            sources_ids = torch.tensor(sources, dtype=torch.long, device=self.args.device)
            source_padding = torch.zeros(padding_length, dtype=torch.long, device=self.args.device)
            sources_ids = torch.cat((sources_ids, source_padding), dim=0)

            # (#doc, 1)
            sources_mask = dataset.post_tweets_masks[idx.item()][:self.relevant_tweets_num].view(-1, 1)
            sources_mask = torch.ones_like(sources_mask)
            sources_mask = torch.cat((sources_mask, source_padding.view(-1, 1)), dim=0)

            tweet_inputs_ids.append(input_ids)
            tweet_masks.append(masks)
            tweet_sources.append(sources_ids)
            tweet_sources_masks.append(sources_mask)

        # (bs, #doc, max_doc_len)
        tweet_inputs_ids = torch.stack(tweet_inputs_ids)
        # (bs, #doc, max_doc_len, 1)
        tweet_masks = torch.stack(tweet_masks)
        # (bs, #doc)
        tweet_sources = torch.stack(tweet_sources)
        # (bs, #doc, 1)
        tweet_sources_masks = torch.stack(tweet_sources_masks)

        # (bs, #doc, max_doc_len, D)
        tweets = self.embedding(tweet_inputs_ids)
        # (bs, #doc, D2)
        tweet_sources = self.tweet_source_emb(tweet_sources)
        tweet_sources = tweet_sources.masked_fill((tweet_sources_masks == 0), 0)
        ...
        df_sizes = tweets.size()
        # (bs * #doc, max_doc_len, D)
        tweet_hiddens = tweets.view(-1, df_sizes[-2], df_sizes[-1])
        # (bs * #doc, max_doc_len, 2H)
        tweet_hiddens, _ = self.tweet_bilstm(tweet_hiddens)
        # (bs, #doc, max_doc_len, 2H)
        tweet_hiddens = tweet_hiddens.view(
            df_sizes[0], df_sizes[1], df_sizes[2], tweet_hiddens.size()[-1])
        ...
        P1 = query_repr.unsqueeze(1).unsqueeze(1).repeat(
            1, tweet_hiddens.shape[1], tweet_hiddens.shape[2], 1)  # [batch_size, #doc, doc_len, hidden_size * 2]
        # [batch_size, #doc, doc_len, hidden_size*4]
        T1 = torch.cat((tweet_hiddens, P1), dim=-1)
        # [batch_size, #doc, doc_len, head_num_1]
        T1 = self.W8(torch.tanh(self.W7(T1)))
        # exclude the padding words in each doc
        T1 = F.softmax(T1, dim=-1)  # [batch_size, #doc, doc_len, head_num_1]
        T1 = T1.masked_fill((tweet_masks == 0), 0)
        # [batch_size * #doc, doc_len, head_num_1]
        T1_tmp = T1.reshape(-1, T1.shape[-2], T1.shape[-1])
        # [batch_size * #doc, doc_len, hidden_size * 2]
        tweet_hiddens_tmp = tweet_hiddens.reshape(-1, tweet_hiddens.shape[-2], tweet_hiddens.shape[-1])
        # [batch_size*#doc, head_num_1, doc_len] * [batch_size*#doc, doc_len, hidden_size * 2]
        E = torch.bmm(T1_tmp.permute(0, 2, 1), tweet_hiddens_tmp)
        # [batch_size, #doc, head_num_1 * hidden_size * 2]
        E = E.view(T1.shape[0], T1.shape[1], -1)
        ...
        # [batch_size, #tweet, head_num_1 * hidden_size * 2 + one_hot_dim]
        E = torch.cat((E, tweet_sources), dim=-1)
        ...
        # [batch_size, #doc, hidden_size * 2]
        P2 = query_repr.unsqueeze(1).repeat(1, E.shape[1], 1)
        # [batch_size, #doc, (head_num_1 + 1) * hidden_size * 2]
        T2 = torch.cat((E, P2), dim=-1)
        T2 = self.W10(torch.tanh(self.W9(T2)))  # [batch_size, #doc, head_num_2]
        # [batch_size, #doc, 1]
        T2 = F.softmax(T2, dim=-2)  # [batch_size, #doc, head_num_2]
        for i in range(len(idxs)):
            if idxs[i] == 3:
                print(T2[i])

        # [batch_size, #doc, head_num_2] * [batch_size, #doc, head_num_1 * hidden_size * 2]
        E = torch.bmm(T2.permute(0, 2, 1), E)
        # [batch_size, head_num_2 * head_num_1 * hidden_size * 2] Eq.(9)
        ...
        if self.only_article:
            ...
            output = torch.cat((query_repr, E), dim=-1)
        else:
            output = torch.cat((query_repr, E, D), dim=-1)
        # (bs, 2H)
...
if __name__ == "__main__":
    ...
    train_file = args.folder + 'dataset/{}/mapped_data/train.json'.format(args.dataset)
    val_file = args.folder + 'dataset/{}/mapped_data/val.json'.format(args.dataset)
    test_file = args.folder + 'dataset/{}/mapped_data/test.json'.format(args.dataset)

    train_post = args.folder + 'dataset/{}/pkl/train_post.pkl'.format(args.dataset)
    val_post = args.folder + 'dataset/{}/pkl/val_post.pkl'.format(args.dataset)
    test_post = args.folder + 'dataset/{}/pkl/test_post.pkl'.format(args.dataset)

    train_article = args.folder + 'dataset/{}/pkl/train_article.pkl'.format(args.dataset)
    val_article = args.folder + 'dataset/{}/pkl/val_article.pkl'.format(args.dataset)
    test_article = args.folder + 'dataset/{}/pkl/test_article.pkl'.format(args.dataset)

    train_tweet = args.folder + 'dataset/{}/pkl/train_tweet.pkl'.format(args.dataset)
    val_tweet = args.folder + 'dataset/{}/pkl/val_tweet.pkl'.format(args.dataset)
    test_tweet = args.folder + 'dataset/{}/pkl/test_tweet.pkl'.format(args.dataset)
    ...
    tweetSource2idx = get_tweet_source(train_dataset)
    args.num_tweet_source = len(tweetSource2idx) + 1
    print('there are {} article sources'.format(len(tweetSource2idx)))

    train_dataset = DatasetLoader(args, train_dataset, train_post, train_article, train_tweet)
    val_dataset = DatasetLoader(args, val_dataset, val_post, val_article, val_tweet)
    test_dataset = DatasetLoader(args, test_dataset, test_post, test_article, test_tweet)
        ...
        if val_result > best_val_result:
            ...
            if args.save_model:
                torch.save({
                    ...
                    os.path.join(args.save, 'fold_{}.pt'.format(epoch))
                )

                if best_val_epoch != -1:
                    os.system('rm {}'.format(os.path.join(
                        args.save, 'fold_{}.pt'.format(best_val_epoch))))
    ...
    final_result = get_metric(best_test_output, test_dataset.labels, get_all=True)
