...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='FakeNewsNet_plus', choices=['FakeNewsNet_plus'])
    ...
    for t in ['train', 'val', 'test']:
        ...
        file = 'dataset/{}/mapped_data/{}.json'.format(args.dataset, t)
        dataset = init_dataset(file)

        posts = text2token([dataset[p]['claim'] for p in dataset], word2idx)

        with open(os.path.join(args.save_dir, '{}_post.pkl'.format(t)), 'wb') as f:
            pickle.dump(posts, f)
        ...
        with open(os.path.join(args.save_dir, '{}_article.pkl'.format(t)), 'wb') as f:
            pickle.dump(post_articles, f)

        post_tweets = dict()
        for post_idx, p in enumerate(dataset):
            tweets = dataset[p]['tweet']
            article_tokens = text2token(tweets, word2idx)
            post_tweets[post_idx] = article_tokens

        with open(os.path.join(args.save_dir, '{}_tweet.pkl'.format(t)), 'wb') as f:
            pickle.dump(post_tweets, f)
